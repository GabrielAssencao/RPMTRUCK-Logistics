...
export async function GET(request: NextRequest) {
  ...
  const [custos, veiculos] = await prisma.$transaction([
  ...
  const porVeiculo = veiculos.map(veiculo => ({ veiculo: veiculo.placa, combustivel: custos.filter(c => c.veiculoId === veiculo.id && c.categoria === 'COMBUSTIVEL').reduce((s, c) => s + c.valor, 0), manutencao: custos.filter(c => c.veiculoId === veiculo.id && c.categoria === 'MANUTENCAO').reduce((s, c) => s + c.valor, 0) }))
  return NextResponse.json({ eficiencia, porVeiculo })
}
