...
test('sidebars permanecem ancoradas e o chat não move a página em atualizações silenciosas', () => {
  ...
  for (const layout of [empresa, admin]) {
    ...
    assert.match(layout, /expandida \? 'h-20' : 'h-16'/)
    assert.match(layout, /expandida \? '' : 'pt-0'/)
    assert.doesNotMatch(layout, /-mx-4 -mt-4 mb-6 flex h-16/)
  ...
  assert.match(brandMark, /src="\/logos\/RpmShieldIcon\.svg"/)
  assert.match(brandMark, /ICON_HUE_ROTATIONS[\s\S]*hue-rotate/)
  assert.match(brandMark, /export function SidebarBrandIdentity/)
  assert.match(brandMark, /h-full min-w-0 items-center/)
  assert.doesNotMatch(brandMark, /subtitle/)
  assert.match(brandMark, /<BrandLogo variant="wordmark" primary=\{primary\}/)
  assert.match(brandMark, /import Image from 'next\/image'/)
