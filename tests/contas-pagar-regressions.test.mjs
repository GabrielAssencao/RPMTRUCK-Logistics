...
test('PDF usa texto e leitura visual com fallback para código de barras', () => {
  ...
  assert.match(page, /useSyncExternalStore\(observarDispositivo, detectarCelular, dispositivoDesktopNoServidor\)/)
  assert.match(page, /capacidades\?\.leituraAutomatica && dispositivoCelular/)
  assert.match(page, /leitura ao vivo e a fotografia do código ficam disponíveis ao acessar esta tela pelo celular/)
...
test('exclusão da empresa exige backup assinado, reautenticação e limpeza recuperável', () => {
  ...
  assert.match(deletion, /empresa\.update\(\{[\s\S]*excluidoEm: agora/)
  assert.match(deletion, /usuario\.update\(\{[\s\S]*email: `removido\+/)
  assert.doesNotMatch(deletion, /eventoSeguranca\.deleteMany/)
  assert.doesNotMatch(deletion, /auditoriaLog\.deleteMany/)
  assert.doesNotMatch(deletion, /empresa\.delete\(/)
  assert.doesNotMatch(exportRoute, /senha_hash:\s*true/)
  assert.match(exportRoute, /where: \{ empresaId, excluidoEm: null \}/)
})
